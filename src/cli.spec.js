...
test("it prints the deployment commands in dry-run mode", () => {
  ...
  assert.deepStrictEqual(result.stdout.trim().split("\n"), [
    ...
    `ssh -l user s1.mydevil.net 'cd ~/domains/example.com/public_nodejs && missing=$(for key in $(grep -E "^[A-Za-z_][A-Za-z0-9_]*=" .env.example 2>/dev/null | cut -d= -f1); do grep -q "^$key=" .env || echo $key; done); if [ -n "$missing" ]; then for key in $missing; do grep "^$key=" .env.example >> .env; done; echo "added missing keys to ~/domains/example.com/public_nodejs/.env: $missing - fill in real values" >&2; exit 1; fi' || ([ -t 0 ] && ssh -t -l user s1.mydevil.net 'cd ~/domains/example.com/public_nodejs && \${EDITOR:-nano} .env' || (echo "filled ~/domains/example.com/public_nodejs/.env with missing keys from .env.example - fill in real values on the server and redeploy" >&2; exit 1))`,
